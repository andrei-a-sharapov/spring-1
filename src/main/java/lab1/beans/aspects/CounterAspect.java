...
public class CounterAspect {
    ...
    @Pointcut("(execution(* lab1.beans.services.EventService.getEvent(String, ..)) && args(eventName, ..)) || "
              + "execution(* lab1.beans.services.EventService.getByName(String)) && args(eventName))")
    private void accessedByName(String eventName) {
    }

    @Pointcut("(execution(* lab1.beans.services.BookingService.getTicketPrice(String, ..)) && args(eventName,..))")
    private void getPriceByName(String eventName) {
    }

    @Pointcut("(execution(* lab1.beans.services.BookingService.bookTicket(*, lab1.beans.models.Ticket)) && args(*, ticket))")
